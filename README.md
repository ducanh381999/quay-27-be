...
- `GET /api/customers?sheetDate=yyyy-MM-dd&queueId=` – list theo ngày; `queueId=1` = chỉ khách đang trong **Quầy 27**  
...
|--------|------|----------------|
| `GET` | `/api/users/sheet-pickers` | Active users for sheet dropdowns: `{ "username", "fullName" }[]` (NV soạn / NV lắp CM on the FE). |
