...
- `GET /api/products/price-lists/items?priceListIds=` – lấy danh sách sản phẩm theo một hoặc nhiều bảng giá
- `POST /api/products/price-lists/{id}/items/add-all` – thêm toàn bộ hàng hóa vào bảng giá
- `POST /api/products/price-lists/{id}/items/add-by-groups` – thêm hàng theo nhóm (body: `{ \"groupIds\": [\"...\"] }`)
